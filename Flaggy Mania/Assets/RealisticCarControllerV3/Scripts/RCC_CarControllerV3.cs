...
public class RCC_CarControllerV3 : RCC_Core {
	...
	public bool useSteeringSmoother = false;
	[Range(0f, 1f)]public float counterSteeringFactor = .5f;				// Counter steering multiplier.
	[Range(10f, 50f)] public float steeringSmootherFactor = 15f;			// Steering smoother multiplier.
	...
	private void Inputs() {
		...
		if (canControl) {
			...
			if (!externalController) {
				...
				if (useSteeringSmoother)
					steerInput = Mathf.Lerp(steerInput, inputs.steerInput + (useCounterSteering ? (driftAngle * counterSteeringFactor) : 0f), Time.deltaTime * steeringSmootherFactor);
				else
					steerInput = inputs.steerInput + (useCounterSteering ? (driftAngle * counterSteeringFactor) : 0f);
